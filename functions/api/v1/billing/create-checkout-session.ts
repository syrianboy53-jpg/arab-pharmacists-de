    ...
    if (!isMock) {
      try {
        ...
        if (stripeRes.ok) {
          ...
          console.error(`Stripe API call failed: ${errText}. Falling back to mock checkout.`)
        }
      } catch (stripeError: any) {
        console.error(`Stripe connection failed: ${stripeError.message}. Falling back to mock checkout.`)
      }
    }

    // Fallback Mock Upgrade Flow
