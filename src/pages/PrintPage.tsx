export default function PrintPage() {
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm text-gray-500 dark:text-gray-400">ملخصات جاهزة للطباعة — اضغط الزر وطبّع</p>
      <div className="grid sm:grid-cols-2 gap-3">
        {[
          ...
          <div key={i} className="bg-white dark:bg-[#1a1a2e] rounded-xl p-5 border border-gray-100 dark:border-white/5">
            ...
            <ul className="space-y-1 mb-4">{card.items.map((item, j) => (<li key={j} className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1.5">• {item}</li>))}</ul>
            <button onClick={() => window.print()} className="w-full bg-gray-100 dark:bg-white/5 text-gray-700 dark:text-gray-300 py-2 rounded-lg text-xs font-bold cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-50 dark:hover:bg-white/10 transition-colors">🖨️ طباعة</button>
          </div>
        ))}
      </div>
    </div>
